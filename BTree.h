//
// Created by leonf on 9/08/2023.
//

#ifndef BD2_BPLUSTREE_H
#define BD2_BPLUSTREE_H
...
struct Item{
    ...
    std::string route;
    Item() : key(T()), route("") {}
    Item(T _key, std::string _value) : key(_key), route(_value) {}
};
...
public:
    Node(std::size_t _degree) {// Constructor
        ...
        Item<T>* _item = new Item<T>[degree-1];
...
public:
    ...
    Node<T>* BPlusTreeSearch(Node<T>* node, T key){
        if(node == nullptr) { // if root is null, return nullptr
            ...
            std::cout<<"Proceso busqueda: "<<std::endl;
            while(!cursor->is_leaf){ // until cusor pointer arrive leaf
                for(int i=0; i<cursor->size; i++){ //in this index node, find what we want key
                    std::cout<<"["<<key<<"<"<<cursor->item[i].key<<"]"<<std::endl;
                    if(key < cursor->item[i].key){ //find some range, and let find their child also.
            ...
            std::cout<<"[Iterar keys del nodo]"<<std::endl;
            for(int i=0; i<cursor->size; i++){
                std::cout<<"["<<key<<" == "<<cursor->item[i].key<<"?]"<<std::endl;
                if(cursor->item[i].key == key){
    ...
    int find_index(Item<T>* arr, Item<T> data, int len){
        int index = 0;
        for(int i=0; i<len; i++){
            if(data.key < arr[i].key){
                index = i;
                break;
            }
            if(i==len-1){
    ...
    Item<T>* item_insert(Item<T>* arr, Item<T> data, int len){
        int index = 0;
        for(int i=0; i<len; i++){
            if(data.key < arr[i].key){
                index = i;
    ...
    Node<T>* child_item_insert(Node<T>* node, Item<T> data, Node<T>* child){
        int item_index=0;
        int child_index=0;
        for(int i=0; i< node->size; i++){
            if(data.key < node->item[i].key){
                item_index = i;
                child_index = i+1;
                break;
            }
    ...
    void InsertPar(Node<T>* par,Node<T>* child, Item <T> data){
        ...
        if(cursor->size < this->degree-1){//not overflow, just insert in the correct position
            ...
            //copy item
            Item<T> * item_copy = new Item<T>[cursor->size+1];
            for(int i=0; i<cursor->size; i++){
            ...
            Item <T> paritem = item_copy[this->degree/2];
    ...
    void insert(Item<T> data) {
        ...
        if(this->root == nullptr){ //if the tree is empty
            ...
            cursor = BPlusTreeRangeSearch(cursor, aux);
        ...
        if(cursor->size < degree/2){//underflow case
            ...
            if(left >= 0){// left_sibiling exists
                ...
                if(leftsibling->size > degree/2){ //if data number is enough to use this node
                    Item<T>* temp = new Item<T>[cursor->size+1];
    ...
    void Removepar(Node<T>* node, int index, Node<T>* par){
        Node<T>* remover = node;
        Node<T>* cursor = par;
        T target = cursor->item[index].key;
        ...
        if(cursor->size < degree/2){//underflow case
            ...
            if(right<=cursor->parent->size){ // right_sibiling exists
                ...
                //merge two leaf node
                for(int i=0; i<rightsibling->size; i++){
                    cursor->item[cursor->size+i+1]=rightsibling->item[i];
                }
                for(int i=0; i<rightsibling->size+1;i++){
    ...
    void bpt_print(std::ostream &os){
        print(this->root,os);
    }
    void print(Node<T>* cursor,std::ostream &os) {
        ...
        if (cursor != NULL) {
            ...
            for (int i = 0; i < cursor->size; ++i) {
                if(cursor->is_leaf){
                    temp = true;
                    os <<cursor->item[i].key << " " << cursor->item[i].route ;
                }
                if(temp){
                    os<<"\n";
                    temp = false;
    ...
    void print(Node<T>* cursor) {
        ...
        if (cursor != NULL) {
            for (int i = 0; i < cursor->size; ++i) {
                std::cout << cursor->item[i].key << "-" << cursor->item[i].route << "   ";
            }
            std::cout << "\n";
    ...
    Item<T> *searchItemById(T key){
        Node <int> *nodde = BPlusTreeSearch(root,key);
        for(int i=0;i<nodde->size;i++){
            if(nodde->item[i].key == key) return &nodde->item[i];
        }
        return 0;
    }
    Node <T> *searchItemByRange(T key){
        return BPlusTreeRangeSearch(root,key);
    }
};
#endif //BD2_BPLUSTREE_H